	...
	public function responseAction() 
    {
		...
		if ($this->getRequest()->get("XMLRES")) 
		{
			...
			$voucher =
				array(
					'_secure' => true,
					'valid' =>(bool)$datapro[10],
					'css' => '../css/bcommerce.css',
					'terminal'=>$datapro[2],
					'empresa'=>Mage::app()->getStore()->getFrontendName(),
					'codunico'=>$codcomers,
					'numtra'=>$datapro[1],
					'fectra'=>$split[0],
					'hortra'=>$split[1],
					...
					'valtot'=>' '.$valor_total,
					'iva'=>' '.$valor_iva,
					'basereturn'=>' '.$valor_return,
					'valnet'=>' '.$neto,
					'descr'=>str_replace( '#', 'No.',(string)$additionalObservations),
					'respuesta'=>$datapro[7],
					'numaut'=>$datapro[3],
					'desresp'=>$datapro[8],
					'cuotas'=>(string)$pay->quotaCode,
					'tarjeta'=>'************'.$tarjeta,
					'codunicocomercio' => $codunico,
					'cardType'=> (string)$pay->cardType
				);
